...
!function (a) {
	"use strict";
	var b = "Compound", c = "Identifier", d = "MemberExpression", e = "Literal", f = "ThisExpression",
		g = "CallExpression", h = "UnaryExpression", i = "BinaryExpression", j = "LogicalExpression", k = !0,
		l = {"-": k, "!": k, "~": k, "+": k}, m = {
			"||": 1,
			"&&": 2,
			"|": 3,
			...
			return c;
		}, o = n(l), p = n(m), q = {"true": !0, "false": !1, "null": null}, r = "this", s = function (a) {
			return m[a] || 0;
		}, t = function (a, b, c) {
			var d = "||" === a || "&&" === a ? j : i;
			return {type: d, operator: a, left: b, right: c};
		}, u = function (a) {
			return a >= 48 && 57 >= a;
		}, v = function (a) {
			...
			for (var i, j, k = 0, n = a.charAt, x = a.charCodeAt, y = function (b) {
				return n.call(a, b);
			}, z = function (b) {
				return x.call(a, b);
			}, A = a.length, B = function () {
				for (var a = z(k); 32 === a || 9 === a;) a = z(++k)
			}, C = function () {
				B();
				for (var b = a.substr(k, p), c = b.length; c > 0;) {
					if (m.hasOwnProperty(b)) return k += c, b;
					b = b.substr(0, --c);
				}
				return !1;
			}, D = function () {
				var a, b, c, d, e, f, g, h;
				if (f = E(), b = C(), !b) return f;
				if (e = {
						value: b,
						prec: s(b)
					}, g = E(), !g) throw new Error("Expected expression after " + b + " at character " + k);
				for (d = [f, e, g]; (b = C()) && (c = s(b), 0 !== c);) {
					for (e = {
						value: b,
						prec: c
					}; d.length > 2 && c <= d[d.length - 2].prec;) g = d.pop(), b = d.pop().value, f = d.pop(), a = t(b, f, g), d.push(a);
					if (a = E(), !a) throw new Error("Expected expression after " + b + " at character " + k);
					d.push(e), d.push(a);
				}
				for (h = d.length - 1, a = d[h]; h > 1;) a = t(d[h - 1].value, d[h - 2], a), h -= 2;
				return a;
			}, E = function () {
				var b, c, d;
				if (B(), b = z(k), u(b) || 46 === b) return F();
				if (39 === b || 34 === b) return G();
				if (v(b)) return J();
				if (40 === b) return K();
				for (c = a.substr(k, o), d = c.length; d > 0;) {
					if (l.hasOwnProperty(c)) return k += d, {type: h, operator: c, argument: E(), prefix: !0};
					c = c.substr(0, --d);
				}
				return !1;
			}, F = function () {
				for (var a = ""; u(z(k));) a += y(k++);
				if ("." === y(k)) for (a += y(k++); u(z(k));) a += y(k++);
				if ("e" === y(k) || "E" === y(k)) {
					for (a += y(k++), ("+" === y(k) || "-" === y(k)) && (a += y(k++)); u(z(k));) a += y(k++);
					if (!u(z(k - 1))) throw new Error("Expected exponent (" + a + y(k) + ") at character " + k);
				}
				if (v(z(k))) throw new Error("Variable names cannot start with a number (" + a + y(k) + ") at character " + k);
				return {type: e, value: parseFloat(a), raw: a};
			}, G = function () {
				...
				if (!d) throw new Error('Unclosed quote after "' + b + '"');
				return {type: e, value: b, raw: c + b + c};
			}, H = function () {
				var b, d = z(k), g = k;
				if (!v(d)) throw new Error("Unexpected " + y(k) + "at character " + k);
				for (k++; A > k && (d = z(k), w(d));) k++;
				return b = a.slice(g, k), q.hasOwnProperty(b) ? {
					type: e,
					value: q[b],
					raw: b
				} : b === r ? {type: f} : {type: c, name: b};
			}, I = function () {
				for (var a, c, d = []; A > k;) {
					if (B(), a = y(k), ")" === a) {
						k++;
					...
					if ("," === a) k++; else {
						if (c = D(), !c || c.type === b) throw new Error("Expected comma at character " + k);
						d.push(c);
					}
				}
				return d;
				...
				for (b = H(), B(), a = y(k); "." === a || "[" === a || "(" === a;) {
					if ("." === a) k++, B(), b = {
						...
						if (c = k, k++, b = {
								...
								object: b,
								property: D()
							}, B(), a = y(k), "]" !== a) throw new Error("Unclosed [ at character " + k);
						k++, B();
					} else "(" === a && (k++, b = {type: g, arguments: I(), callee: b});
					B(), a = y(k);
				}
				return b;
			}, K = function () {
				...
				var a = D();
				if (B(), ")" === y(k)) return k++, a;
				throw new Error("Unclosed ( at character " + k);
			}, L = []; A > k;) if (i = y(k), ";" === i || "," === i) k++; else if (j = D()) L.push(j); else if (A > k) throw new Error("Unexpected '" + y(k) + "' at character " + k);
	...
	if (x.version = "0.2.9", x.toString = function () {
		...
		var y = a.jsep;
		a.jsep = x, x.noConflict = function () {
...
jsep = window.jsep ||
	...
	typeof exports !== undefined && exports.parse;
